...
import { Stage2ApplyService } from './stage2-apply.service';
...
export class AnalysisService {
  ...
  async startAnalysis(dto: StartAnalysisDto): Promise<{ projectId: string; status: string }> {
    ...
    this.processAnalysis(
      projectId,
      effectiveStage1Model,
      effectiveStage2Model,
  ...
  private async processAnalysis(
    projectId: string,
    stage1Model: string, // ← теперь точно string
    stage2Model: string, // ← теперь точно string
    reasoningEffort: 'low' | 'medium' | 'high', // ← теперь точно литерал
  ): Promise<void> {
  ...
  private assembleFinalLaw(changes: AppliedChange[]): string {
