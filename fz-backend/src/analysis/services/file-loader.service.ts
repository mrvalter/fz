...
export class FileLoaderService {
    ...
    constructor(private readonly configService: ConfigService, private readonly docXService: DocxService) {
    ...
    async loadProjectLawText(projectId: string): Promise<string> {
        const filePath = path.join(this.uploadsPath, 'projects', projectId, 'project.docx');
    ...
    private async extractTextFromFile(filePath: string): Promise<string> {
        ...
        try {
            ...
            const textFromBuffer = this.docXService.extractTextFromBuffer(dataBuffer);
